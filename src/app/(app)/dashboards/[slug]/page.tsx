...
import { Wrench, CalendarIcon, Loader2, X } from "lucide-react";
...
import { startOfMonth, endOfMonth, addMonths, subMonths, subDays, addDays, format, isBefore, startOfDay } from 'date-fns';
    ...
    return (
        <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="balance" name="Saldo" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
            </LineChart>
        </ResponsiveContainer>
    );
}
...
function FinancialDashboard() {
    ...
    return (
        <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
                            ...
                            <Popover>
                                ...
                                <PopoverContent className="flex w-auto flex-col sm:flex-row p-0" align="start">
                                    <div className="flex flex-col gap-1 border-r p-3">
                                        ...
                                        <Button variant="ghost" className="w-full justify-start px-2" onClick={() => setDate({from: subDays(new Date(), 89), to: new Date()})}>Últimos 90 dias</Button>
                                        <Button variant="ghost" className="w-full justify-start px-2" onClick={() => setDate({from: subDays(new Date(), 29), to: new Date()})}>Últimos 30 dias</Button>
                                        <Button variant="ghost" className="w-full justify-start px-2" onClick={() => { const now = new Date(); setDate({from: startOfMonth(subMonths(now, 1)), to: endOfMonth(subMonths(now, 1))})}}>Mês anterior</Button>
                                        ...
                                        <Button variant="ghost" className="w-full justify-start px-2" onClick={() => { const now = new Date(); setDate({from: startOfMonth(addMonths(now, 1)), to: endOfMonth(addMonths(now, 1))})}}>Mês seguinte</Button>
                                        <Button variant="ghost" className="w-full justify-start px-2" onClick={() => setDate({from: new Date(), to: addDays(new Date(), 29)})}>Próximos 30 dias</Button>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Fluxo de Caixa (Histórico e Projeção)</CardTitle>
