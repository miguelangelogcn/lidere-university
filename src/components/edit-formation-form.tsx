  ...
  const onSubmit = async (data: FormationFormValues) => {
    const finalData = {
        ...
        modules: data.modules.map((module, moduleIndex) => ({
            ...module,
            id: module.id || doc(collection(db, '_')).id,
            order: moduleIndex,
            lessons: module.lessons.map((lesson, lessonIndex) => ({
                ...lesson,
                id: lesson.id || doc(collection(db, '_')).id,
                order: lessonIndex,
        ...
        <ScrollArea className="h-[65vh] pr-6">
            <div className='space-y-4'>
                <FormField control={form.control} name="title" render={({ field }) => (
                    <FormItem><FormLabel>Título da Formação</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                ...
                <Button type="button" variant="outline" onClick={() => appendModule({ id: doc(collection(db, '_')).id, title: '', lessons: [{ id: doc(collection(db, '_')).id, title: '', textContent: '', videoUrl: '', attachments: [] }] })}>
