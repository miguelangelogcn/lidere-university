...
type AddPipelineFormProps = {
  onSuccess: () => void;
};

export function AddPipelineForm({ onSuccess }: AddPipelineFormProps) {
  ...
  const form = useForm<PipelineFormValues>({
    ...
    defaultValues: {
      name: '',
      stages: [{ name: 'Qualificação' }, { name: 'Proposta' }, { name: 'Negociação' }],
    },
  });
  ...
  const onSubmit = async (data: PipelineFormValues) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Erro!", description: err.message || 'Falha ao criar funil.' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ex: Funil de Vendas Padrão" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
