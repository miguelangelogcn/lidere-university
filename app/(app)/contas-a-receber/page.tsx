...
export default function ContasAReceberPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [accountsData, companiesData] = await Promise.all([
                getAccounts('receivable', selectedCompanyId),
                getCompanies()
            ]);
    ...
    const handleMarkAsPaid = async (account: SerializableAccount) => {
        try {
            await updateAccount('receivable', account.id, { status: 'paid', paidAt: new Date() });
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            await deleteAccount('receivable', accountForAction.id);
        ...
        <>
            <MainHeader title="Contas a Receber">
                <Dialog open={isFormOpen} onOpenChange={(open) => { setIsFormOpen(open); if (!open) setAccountForAction(null); }}>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{accountForAction ? 'Editar' : 'Adicionar'} Conta a Receber</DialogTitle>
                            <DialogDescription>Preencha os detalhes da receita.</DialogDescription>
                        </DialogHeader>
                        <AccountForm accountType="receivable" account={accountForAction} onSuccess={handleSuccess} />
                    </DialogContent>
                </Dialog>
            </MainHeader>
            <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total a Receber (Pendente)</CardTitle></CardHeader>
                        <CardContent><div className="text-2xl font-bold text-green-600">{loading ? <Skeleton className="h-8 w-32" /> : formatCurrency(totalPending)}</div></CardContent>
                    </Card>
                ...
                <Card>
                    <CardHeader><CardTitle>Lista de Contas a Receber</CardTitle><CardDescription>Contas pendentes e recebidas da empresa selecionada.</CardDescription></CardHeader>
                    <CardContent>
                                                ...
                                                <TableRow key={account.id} className={account.status === 'paid' ? 'bg-muted/50 text-muted-foreground' : ''}>
                                                    ...
                                                    <TableCell>
                                                        <Badge variant={account.status === 'paid' ? 'secondary' : 'outline'}>
                                                            {account.status === 'paid' ? 'Recebido' : 'Pendente'}
                                                        </Badge>
                                                    </TableCell>
                                                    <TableCell>
                                                        {account.status === 'pending' && (
                                                            <DropdownMenu>
                                                                ...
                                                                <DropdownMenuContent>
                                                                    ...
                                                                    <DropdownMenuItem onSelect={() => handleMarkAsPaid(account)}><Check className="mr-2 h-4 w-4"/> Marcar como Recebida</DropdownMenuItem>
